...
"""
ArchaeologicalMapExtractor_AutoGeoref_v9_gcp_fallback.py
...
def extract_labels(page,bbox):
        ...
        for line in block.get("lines",[]):
            for s in line.get("spans",[]):
                ...
                if not re.fullmatch(r"\d{1,3}",t):
                    continue
                bx=s["bbox"]; x=(bx[0]+bx[2])/2; y=(bx[1]+bx[3])/2
                if not(x0<=x<=x1 and y0<=y<=y1):
                    continue
                if abs(float(s["size"])-8)>0.7:
                    continue
                rows.append({"sequence":seq,"label_no":int(t),"pdf_x":x,"pdf_y":y})
                seq+=1
    ...
    for i,r in enumerate(rows):
        city="74" if start is not None and start<=i<start+26 else "68"
...
def simple_table(page):
    ...
    PyMuPDFの内部テキストを利用し、
      市町村番号(2桁) -> 遺跡番号(3桁)
    が連続する行を一覧表レコードの開始として検出する。

    特定自治体コード(68/74等)には依存しない。
    同じ表がPDF内部で重複している場合は site_uid で重複除去する。

    site_name は遺跡番号の次から最初の「○」までのテキストを連結する。
    これにより、複数行に折り返された遺跡名にも対応する。
    """
    raw_lines = page.get_text("text").splitlines()
    lines = [clean_text(x) for x in raw_lines]

    # row start candidates: "57" followed by "001" のような組合せ
    starts = []
    for i in range(len(lines) - 1):
        city = lines[i]
        no = lines[i + 1]
        if re.fullmatch(r"\d{2}", city) and re.fullmatch(r"\d{3}", no):
            starts.append((i, city, no))
    ...
    # 地図中の数字が偶然 2桁+3桁で並ぶ場合を除外するため、
    # 各municipality_codeについて「ユニークな遺跡番号数」と
    # 1..max_no の連続性を評価し、最も表らしいコードを選ぶ。
    by_code = {}
    for _, city, no in starts:
        by_code.setdefault(city, set()).add(int(no))
    ...
    def code_score(item):
        ...
        for n in range(1, max_no + 1):
            if n in nums:
                ...
                break
        return (len(nums), coverage, consecutive)

    municipality_code, nums = max(by_code.items(), key=code_score)

    # 選択自治体コードのrow startだけ残す。
    starts = [x for x in starts if x[1] == municipality_code]

    records = []
    for j, (i, city, no) in enumerate(starts):
        next_i = starts[j + 1][0] if j + 1 < len(starts) else len(lines)
        tokens = [x for x in lines[i + 2:next_i] if x]

        # 名称は最初の○より前。表の遺跡名が複数行でも連結する。
        name_parts = []
        for tok in tokens:
            if tok == "○":
                break
        ...
        rec = {
            ...
            "site_uid": f"{city}-{no}",
            "site_name": site_name,
            "raw_row_text": " ".join([city, no] + tokens[:30]),
        }
        records.append(rec)
    ...
    # PDF内部に同一表が複数回含まれるケースをsite_uidで統合。
...
def choose_gcp_zoom(bbox, preferred=15, max_tiles=500):
    """手動GCP用に、広域を表示できる最大zoomを選ぶ。"""
    for z in range(int(preferred), 7, -1):
...
def interactive_gcp_gui(raster, mosaic, csv_path, title_suffix=""):
    ...
    ax_gsi.set_title("← 同じ地点を地理院地図でクリック")
...
def prepare_manual_gcp_mosaic(search_bbox, outdir, layer="pale",
                              preferred_zoom=15, max_tiles=500):
    z=choose_gcp_zoom(search_bbox,preferred_zoom,max_tiles)
    print(
        f"  手動GCP参照地図: layer={layer}, z={z}, "
        f"tiles={tile_count_for_bbox(z,search_bbox)}"
    )
    mos=build_mosaic(layer,z,search_bbox,Path(outdir)/"gsi_tiles")
    cv2.imwrite(str(Path(outdir)/"05_gcp_reference.png"),mos.img)
    return mos


def manual_gcp_fallback(raster, search_bbox, outdir, args, municipality_name=None):
    """
...
def parse_args():
    ...
    p.add_argument("--gcp-zoom",type=int,default=15,
                   help="手動GCP参照地図の希望zoom。タイル数超過時は自動で下げる")
    p.add_argument("--gcp-max-tiles",type=int,default=500,
                   help="手動GCP参照モザイクの最大タイル数 (default: 500)")
...
def main():
    ...
    print("=== 3 地図番号 ===")
    labels=extract_labels(page,bbox)
    labels.to_csv(a.out/"02_map_labels_pdf.csv",index=False,encoding="utf-8-sig")
    print(f"{len(labels)} labels / {labels.site_uid.nunique()} unique")
    ...
    master=table.merge(first[["site_uid","lon","lat","pdf_x","pdf_y"]],on="site_uid",how="left")
    master=master.merge(mdf.drop_duplicates("site_uid"),on="site_uid",how="left")
